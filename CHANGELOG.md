...
- A new Shield + Recovery mark gives DeepPurge a distinct identity at taskbar, app, and repository sizes.
- A private-desktop capture tool records the real WPF interface without using the interactive Windows desktop or staged product data.
- The repository now includes current Programs, Junk Cleaner, and Health Dashboard screenshots plus a GitHub-ready hero image.

### Changed

- The README now leads with the product promise, download links, real interface proof, safety model, and a shorter path to the GUI or CLI.
- App chrome, executable resources, package metadata, and release copy now use the same DeepPurge identity and v0.9.2 version.
- Release packaging now focuses on the portable executables and the maintained Scoop manifest.
- Installed-program capability names now read naturally, and the table gives removal decisions and trust signals enough room to scan.

### Fixed

- Collection-backed empty states now react to count changes, so recovery records and scheduled jobs no longer appear beneath a stale empty card.
- The capture utility chooses between native-window and visual-tree rendering after checking the brand region, preventing incomplete sidebar captures on a private desktop.

### Removed

- The repository-authored winget manifest was removed. DeepPurge still discovers and manages software installed through winget.
...
### Added (P1 removal capability)
- **Explicit removal capabilities and trust facts**. Every installed-program row now identifies its source-native removal capability, source identity, executable, arguments, owner, publisher, signature, and risk. Native uninstallers and package-manager commands are revalidated immediately before execution, portable folders use recoverable deletion, game-launcher discoveries remain disabled until a source-native action exists, and unsupported or skipped actions never report success. WPF, CLI, and program exports expose the same review facts.
...
- **15 bundled cleaner definitions for modern apps**: VS Code, Cursor, Windsurf, Discord, Slack, Microsoft Teams, Notion, Obsidian, Figma, Docker Desktop, Zen Browser, Arc Browser, WSL caches, Postman, Spotify. Auto-extracted to `DataPaths.Cleaners/bundled-modern-apps.cleaner.json` on first run. Supplements the stale winapp2.ini (last updated Nov 2025). Users can edit or delete the file.
...
- [ ] P2: Enforce a capability-to-surface and documentation contract
  ...
  Evidence: `src/DeepPurge.App/ViewModels/MainViewModel.Extensions.cs`; `src/DeepPurge.Core/Diagnostics/HealthScorer.cs`; `src/DeepPurge.Core/Cleaning/SystemSlimmer.cs`; `src/DeepPurge.Core/App/AppSettings.cs`; `src/DeepPurge.App/Views/MainWindow.xaml`; `src/DeepPurge.Cli/Program.cs`; `README.md`.
